...
**Dive Deeper:**
From the confusion matrix above, try and answer the following questions. 
...
library(caret)
confusionMatrix(votes_prediction, votes.test$party)
```
...
```
```{r}
#peluang Democrat watercost Y / hcapped N
x <- 0.47 * 0.37 * 0.59 / ((0.47 * 0.37 * 0.59) + (0.45*0.81*0.40))
...
summary(pred_diab_def_train)
summary(pred_diab_def_test)

caret::confusionMatrix(pred_diab_def_train, diab_train[,9]) # column 9 = Diabetes
caret::confusionMatrix(pred_diab_def_test, diab_test[,9]) # column 9 = Diabetes
```
...
```{r eval=F}
set.seed(417)
ctrl <- trainControl(method="repeatedcv", number=5, repeats=3) # method = "repeatedcv" artinya repeated Cross Validation
...
varImp(fb_forest) # Menentukan variable yang paling penting (sering muncul di node)
```
...
```{r}
table(predict(fb_forest, fb_test[,-57]), fb_test[,57]) #QUESTION: Kenapa column 57 ??
```
...
```{r}
plot(fb_forest$finalModel)fb_fore
