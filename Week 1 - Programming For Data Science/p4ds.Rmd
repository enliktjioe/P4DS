...
```{r}
# Your code here
sales_records <- rbind(sales,returns,netsales)
```
...
# Your code here
dim(sales_records)
colnames(sales_records) <- accounts
```
...
# R Programming with Retail Dataset
...
# retail <- read.csv("data_input/retail.csv", header = FALSE, stringsAsFactors = FALSE) 
# extra argument just in case
```
...
order11816 <- subset(retail, retail$Order.Date == "11/11/16")
order11816 <- subset(retail, retail$Order.Date == "11/11/16" | 
                              retail$Order.Date == "12/11/16")
order11816
...
Take all columns, and take all rows except the first (second row to last row)
```{r}
retail[2:9994, ] # This is working as normal, but not the best practice
retail[2:nrow(retail), ] # The best practice for production

retail[-1, ] # except first row
retail[-3, ] # except third row

retail[retail$Profit != 0, c(10, 13:15)]
retail[retail$Profit != 0, c("Sub.Category", "Quantity", "Discount", "Profit")]

retail[retail$Sub.Category == "Chairs", c(10, 13, 15)]
```

How many transactions happen to be losses (Profit < 0) in the Tables sub-category?
```{r}
no_profit <- retail[retail$Profit < 0 & retail$Sub.Category == "Tables", ]
nrow(no_profit)

table(retail$Segment) # Show every segment values
big_profit <- retail[retail$Profit > 1000 & 
                      retail$Segment == "Home Office", ]
nrow(big_profit)
```

How many of our shipments were on First Class?
```{r}
table(retail$Ship.Mode) # 1st method
nrow(retail[retail$Ship.Mode == "First Class", ]) # 2nd method
summary(retail) # 3rd method

summary(retail[ ,1:4]) # summary combine with subsetting
table(retail$Ship.Mode, retail$Segment) # 2-dimensional table
```

How many Furniture did we sell to Home Office customers?
```{r}
summary(retail)
table(retail$Category, retail$Segment) # 1st method
# ANSWER: 362

nrow(retail[retail$Category == "Furniture" & retail$Segment == "Home Office", ]) # 2nd method
```

Proportion of table and Pie Chart
```{r}
table(retail$Sub.Category)
prop.table(table(retail$Sub.Category))

pie(table(retail$Ship.Mode))
```

How many percentage of our sales were shipped to Corporate on First Class?
Tip: `prop.table(table(retail...))`
Create a two-dimensional table, `table(x$x, x$y)`
Wrap the table under `prop.table()`

```{r}
table1 <- table(retail$Segment, retail$Ship.Mode)
prop.table(table1) * 100 # multiply by 100 to format it into percentage
# ANSWER = 0.04852912 (4.85%)
```

Among the shipments to Corporate, how many percent were on first class?
```{r}
## HARD CODE method
table2 <- table(retail$Segment == "Corporate", retail$Ship.Mode)
table2
485/(485+114+609+1812) * 100
# ANSWER = 0.160596 (16.06%)

## Best Practice method - Think first, and start code later
# Subset for Corporate Segment, call it corpsales
corpsales <- retail[retail$Segment == "Corporate",]
corpsales

# Use prop.table on the corpsales
table3 <- table(corpsales$Ship.Mode)
table3
prop.table(table3) * 100

## Another method
table("Corporate Segment" = retail[retail$Segment == "Corporate", "Ship.Mode"])
```
...
retail$Order.Month <- months(retail$Order.Date)
table(retail$Order.Month)

# 1st method
y2016 <- retail[retail$Order.Year == "2016", ]
y2017 <- retail[retail$Order.Year == "2017", ]
summary(y2016$Profit)
summary(y2017$Profit)

# 2nd method (more efficient) - Cross-Tabulation (`xtabs`)
str(retail$Ship.Mode)
xtabs(Profit ~ Segment + Ship.Mode, data = retail)

```

```{r}
# 1. Subset for homeOffice
homeOffice <- retail[retail$Segment == "Home Office", ]
homeOffice

# 2. prop.table(table(homeOffice$Ship.Mode))
prop.table(table(homeOffice$Ship.Mode)) * 100
...
A.  Consumer Office Supplies 
B.  Consumer Technology 
C.  Home Office Furniture 
D.  Corporate Technology 
E.  Corporate Furniture 

```{r}
sales_1000_more <- retail[retail$Sales >= 1000,]
sales_1000_more

table(sales_1000_more$Segment, sales_1000_more$Category)
```

2. Among the transactions that ship on 'First Class', how many of them were office supplies (to two decimal points)? 
A.  21.26% 
B.  19.57% 
C.  2.13% 
D.  60.01% 
...
```{r}
str(retail)
firstClass <- retail[retail$Ship.Mode == "First Class", ]
head(firstClass)

prop.table(table(firstClass$Category)) * 100
```

*Ordered Month*
```{r}
retail$Order.Month <- factor(retail$Order.Month, levels = month.name)
table(retail$Order.Month)
```


*Cross Tabulation xtabs() can't used for implement function*
*Use aggregate() instead*
```{r}
# xtabs(formula = Profit ~ Segment + Ship.Mode, data = retail)
aggregate(formula = Profit ~ Segment + Ship.Mode, data = retail, FUN = mean)
```

QUIZ
Use aggregate / xtabs; Which combination of Segment and Shipment mode is the most profitable in year 2016

```{r}
retail$Ship.Date <- as.Date(retail$Ship.Date, "%d/%m/%y")
head(retail$Ship.Date, n = 20)

aggregate(Profit ~ Segment + Ship.Mode, retail[retail$Order.Year == "2016",], FUN = sum)
xtab1 <- xtabs(Profit ~ Segment + Ship.Mode, retail[retail$Order.Year == "2016",])

plot(xtab1)


```
# ANSWER: Consumer - Standard Class
