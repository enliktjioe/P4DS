...
How many of our shipments were on First Class and to Corporate customers?
```{r}
table(retail$Ship.Mode, retail$Segment)
```

1. How many percentage of our sales were shipped to Corporate on First Class?
...
Summary goes through each column, if:
- Numeric: 5 number summary (min, median, max, 1st and 3rd quantile)
- Factor / Categorical: table
...
What is the total Profit across each customer Segment and Shipment Mode? Which is the most profitable?
```{r}
xtabs(Profit ~ Segment + Ship.Mode + Category, retail)
```


1. Subsetting
2. Tables (`table()`) and Proportion Tables (`prop.table(table(x$Ship))`)
3. Cross-Table (`xtabs()`)

Among the Home Office segment, what percent of our sales are shipped on Standard class?
```{r}
...
Which month do we perform better for Technology products?
```{r}
...
```{r}
xtab1 <- xtabs(Profit ~ Sub.Category + Ship.Mode, data=retail[retail$Order.Year == "2016", ])

plot(xtab1, main="Profit Breakdown 2018")
```



```{r}
profit18 <- aggregate(Profit ~ Segment + Ship.Mode, 
          data=retail[retail$Order.Year == "2016", ],
          FUN=sum)
profit18 <- data.frame(profit18)
head(profit18)
```

```{r}
profit18[ order(profit18$Profit, decreasing=TRUE), ]
```

```{r}
retail$Order.Quarter <- quarter(retail$Order.Date)
retail$Order.DOW <- weekdays(retail$Order.Date)
retail$Order.onWeekend <- ifelse(retail$Order.DOW == "Saturday" | retail$Order.DOW == "Sunday", "onWeekend", "onWeekday")
head(retail[,c(3,16:20)])
```

1. Do we do more discounts in Q1 or Q2 .. Q4?
2. Do we do more First Class shipments on weekends than on weekdays?

```{r}
aggregate(Discount ~ Order.onWeekend, retail, FUN=mean)
```

```{r}
aggregate(Ship.Mode ~ Order.Quarter, retail, FUN=mean)
```




1. In 2016, what is our average profit? How does that compare to 2017?
```{r}
date1 <- as.Date("2016-01-01")
date2 <- as.Date("2016-12-31")

date3 <- as.Date("2017-12-31")

y2016 <- retail[ retail$Order.Date >=  date1 & 
                   retail$Order.Date <= date2,  ]

y2017 <- retail[ retail$Order.Date > date2 & 
                   retail$Order.Date <= date3, ]
...
```{r}
electionday <- "18/06/2019"

# transform the above to a Date class
# write your code here:
# tip: use `as.Date(x, format="...")`


# class should print "Date"
class(electionday)
```
