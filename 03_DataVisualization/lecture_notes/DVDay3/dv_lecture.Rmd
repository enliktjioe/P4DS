...
# ydm(vids$trending_date) %>% 
#   as.Date(format = "%y-%d-%m") %>% 
#   head()
# format(ydm(vids$trending_date), "%y/%d/%m") %>% head()
```
...
```{r swich}
x <- c("1","2","3")
#x <- switch(x,"1"="Apel","2"="Mangga")
x <- sapply(x, switch,"1"="Apel","2"="Mangga")
```
...
subset data vids dengan kondisi jumlah `dislikes` terbanyak
```{r latihan}
vids[vids$dislikes==max(vids$dislikes),]
vids[vids$title=="So Sorry.",]

```


```{r lubridate_ex}
vids.max <- vids[vids$dislikes==max(vids$dislikes),]
year(vids.max$trending_date)
month(vids.max$trending_date, label = T, abbr = F)
day(vids.max$trending_date)
weekdays(vids.max$trending_date)
```
...
data1 <- data.frame(x1=c(1,2,1,2), x2 = c("a","b","c","d"))
data <- data1[match(unique(data1$x1),data1$x1),]
data
```
...
```{r}
plot(x = vids.agt$likes, y = vids.agt$dislikes)
...
Here's the code:
```{r}
plot(x = vids.agt$likesp,
     y = vids.agt$dislikesp,
...
abline(h = mean(vids.agt$dislikesp), lty=6, lwd=5, col="red")
abline(v = 0.05)
...
```{r}
plot(vids.agt$likesp, vids.agt$dislikesp, col=vids.agt$category_id, pch=1)
...
plot(x = vids.agt$likesp,
     ...
     ylab = "Dislikes ratio",
     main = "Likes vs Dislikes Ratio")
abline(lm(vids.agt$dislikesp~vids.agt$likesp), col="blue", lwd=1, lty=2)
legend("topright", legend = levels(vids.agt$category_id), fill = 1:3)
```
...
plot(x = vids.agt$likesp, 
     ...
     pch = 19,
     cex = 1,
     main = "like and dislike ratio Youtube",
     sub = "using youtube tranding dataset",
     xlab = "Likes Ratio",
     ylab = "Dislike Ratio",xlim = c(0,0.02), ylim = c(0,0.02))
abline(lm(vids.agt$dislikesp~vids.agt$likesp), lwd=1, lty=4)
legend("topright", legend = levels(vids.agt$category_id), fill = 1:3)
```
...
```{r}
hist(vids.agt$likesp, breaks =c(0.00, 0.05,0.15))
hist(vids.agt$likesp, breaks = 10)
```
...
```{r}
dotchart(vids.ags$likesp, labels=vids.ags$channel_title, cex=1, pch=19, groups=vids.ags$category_id, col=vids.ags$col)
...
`ggplot2` is created by Hadley Wickham in 2005 as an implementation of Lelad Wilkinsonb s Grammar of Graphics. The idea with Grammar of Graphics is to create a formal model for data visualization, by breaking down graphics into components that could be systematically added or subtracted by the end user.
...
Once we created our `ggplot` object (we named it `g`), we can now add a layer onto it using `geom_`. `geom` is ggplotb s way of handling geometry, i.e. how the data are represented on the plot. To illustrate this idea, letb s add a geom_point and then print the resulting object:

```{r}
g + 
  geom_point(aes(size=gempa$size))
```

A recap of what web ve done so far:  

- Creating our ggplot graphics object through `ggplot()`  
- We specify 2 arguments in our call to ggplot; Itb s helpful to note that any argument we pass into `ggplot()` will be used as global options for the plot, i.e. they apply to all layers we add onto that graphics object  
...
- We tell ggplot how we want the graphic objects to be represented by adding (through the b +b  operator) our geom layer. Since we added `geom_point`, this is equivalent to adding a layer of scatterplot to represent our x and y variables  
...
g +
  ...
  labs(title="Disaster Impact Zone, Invasion Area 2018",
       caption="source: Jakarta Distaster Relief", 
       x = "longitude", 
       y = "latitude")
```
...
```{r}
ggplot(data = vids.ags, aes(x = category_id, y = dislikesp))+
  geom_boxplot()+
  geom_jitter(aes(size =comment_count/views, col= category_id ))
  



ggplot(data = vids.ags, aes(x = category_id, y = dislikes))+
geom_jitter(aes(size=comment_count/views) )+
labs(x= "category_id", y = "dislikes")
```
...
Hopefully that was a lot of satisfaction and a fun introduction to ggplot! 
...
`temp1` is a dataframe with two variables, and contain a list of channels that have at least 10 videos being trending in the observation period. Let's create a column chart (`geom_col`) and flip the coordinates (`coord_flip`) so the category names can be displayed horizontally. That makes it easier for the user to read and identify the videos that are more prolific than others in producing trending videos:

```{r}
names(temp1)
ggplot(data = temp1, aes(x =reorder(Var1, Freq) , y=Freq))+
  geom_col()+ 
  coord_flip()
...
Here's the code:
```{r}
aggregate.data.frame(dislikes~category_id,vids.u, mean)
aggregate.data.frame(x = vids.u$dislikes, by= vids.u$category_id, FUN = mean)
temp2 <- aggregate.data.frame(vids.u$dislikes, by=list(vids.u$category_id), mean)
head(temp2)
```


**Dive Deeper**: Can you use `geom_col` and `coord_flip` to create a column chart that plots the amount of average likes by category? Refer to earlier exercises if you need to. Add a `labs()` component to label the x- and y-axis appropriately as a bonus.
```{r}
...
Now what is important here is that the above did not account for the views each of these category has, and so before jumping into any conclusion it's paramount we think with clarity and purpose the "biases" our plot or statistics may show. Perhaps a better idea is to have our dislikes "adjusted for" the views, so it is more representative of the true events!
...
```{r}
#aggregate(dislikesp~category_id, vids.ags, FUN=mean)
...
names(temp3)
ggplot(temp3[1:20,], aes(x=Group.1, y=dislikeratio))+
...
I hope it's obvious here that while we used `coord_flip()` to attain a plot resembling a dot plot, we can apply the `coord_flip` function on any other geoms, and such flexbility clearly has much to offer when the data we want to visualize is relatively large, making the right arrangement of the plot's element especially important. Let's see an example below:
```{r}
ggplot()

ggplot(vids.u[vids.u$ratings_disabled == F, ], aes(x=category_id, y=likes/views))+
  geom_boxplot(aes(col=category_id),show.legend = F)+
  coord_flip()+
  labs(title = "Sebaran likes ratio")
...
To facilitate our tasks, I'm introducing a `package` called `reshape2`, which allow us to reshape a data frame between "wide" (measurement spread across horizontally) and "long" (measurement collected vertically). In the following code, I'm reshaping the `vids.u` dataframe from a wide one to a long one, with `category_id` being the ID and measurements being `likes`, `dislikes` and `comment_count`: 
...
vids.m <- melt(vids.u[vids.u$comments_disabled == F,c(4,7,8,9)], id.vars = "category_id")
...
```{r}
head(vids.m)
ggplot(vids.m, aes(x=category_id , y=value))+
  geom_col(position="fill", aes(fill=vids.m$variable))+
  coord_flip()
...
```{r}
# 4,8,7,9 pointing to category_id, likes, dislikes, comment_count
vids.hate <- melt(vids.u[vids.u$dislikes >= vids.u$likes & vids.u$ratings_disabled == F, c(4,7,8,9)])
```

```{r}
ggplot(vids.hate, aes(x=reorder(category_id, value) , y=value))+
  geom_col(position="dodge", aes(fill=vids.hate$variable))+
  coord_flip()
...
```{r}
vids.hate2 <- melt(vids.u[vids.u$dislikes >= 10000 &
...
Daily Caller is a far-right news and opinion website that posted a controversial and potentially offensive video titled "PSA from Chairman of the FCC Ajit Pai". In this video, the FCC Chairman dismisses concerns over net neutrality while dancing around in a Santa suit to b Harlem Shakeb  and swinging a lightsaber. Logan Paul, back in December 2017 published a disturbing video titled "Found Dead Body in Forest" that featured actual footage of what appears to be a dead body when he and his friends ventured into Aokigahara (known also as the Suicide Forest) and subsequently a video titled "so sorry". 

Washington Post's "The FCC repeals its net neutrality rules" was third on the most disliked trending videos, and it is actually a 3-hour long live stream in which federal regulators vote to allow Internet providers to speed up service for some apps and websites b  and block or slow down others. The decision is considered highly unpopular.
...
ggplot(vids.u[vids.u$channel_title == "Universal Pictures", ], aes(x=trending_date, y=likes/dislikes, color=comment_count, size=views)) +
  ...
  geom_label(aes(label=title), nudge_y = 1, size=3, label.size = 1, hjust = "inward")+
...
coba buat bar plot antara  `title` dan `views` pada video yang  `channel_title` == "INSIDER" lalu tambahkan labs
...
```{r jawaban}
ggplot(vid.in,aes(x= reorder(title, views), y = views))+
geom_col()+
coord_flip()+
  labs(title = "Jumlah Views Channel INSIDER",
       x = "Judul Video",
...
ggplot(data = vids.ags, aes(x=category_id, y=dislikes, size=comment_count/views))+
  geom_jitter(aes(col=publish_wday))+
  theme_wsj(base_size = 8,color = "green")+
...
Cleveland (1985), page 264: b Data that can be shown by pie charts always can be shown by a dot chart. This means that judgements of position along a common scale can be made instead of the less accurate angle judgements.b  This statement is based on the empirical investigations of Cleveland and McGill as well as investigations by perceptual psychologists."
