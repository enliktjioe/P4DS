...
```{r}
x <- c("1", "2","1")
# switch(x, "1" = "Mangga","2"="jambu") # masih error 
sapply(x, switch, "1" = "Mangga","2"="jambu")
```
...
# CTRL + D = shortcut for DELETE 1 line of code
```
...
Tugas: Subset data vids dengan kondisi dislike terbanyak
```{r}
str(vids)
most_dislikes <- vids[vids$dislikes == max(vids$dislikes), ]
head(most_dislikes)
```
...
# Analysis: Apakah waktu menonton (dalam jam) mempengaruhi rasio *like* ?
```
...
# Finish Day 1
# Resolved error: plot.new has not been called yet
...
Learn by doing
```{r}
plot(x = vids.agt$likesp, y = vids.agt$dislikesp, col = vids.agt$category_id,
      xlab = "Likes Ratio", ylab = "Dislikes Ratio", main = "Likes vs Dislikes Ratio")
legend("topright", legend = levels(vids.agt$category_id), fill = 1:3)
abline(h = 0.005, v = 0.05, lty = 5, lwd = 5, col = "red")
```
...
```{r}
g + geom_point(aes(size = gempa$size))
```
...
ggplot(data = vids.ags, aes(x=category_id, y=dislikes, size=comment_count/views))+
  ...
  labs(title = "Dislikes to View Comparison", subtitle = "YouTube Trending Data, 2018", x = "Video Category",
       y = "Dislikes/View", caption = "Made by Enlik")
...
vids.m <- melt(vids.u[vids.u$comments_disabled == F,c(4,7,8,9)])
rbind(head(vids.m), tail(vids.m))
```

```{r}
laporan <- data.frame(produk = c("Apel", "Pisang", "Melon"),
                      tahun2015 = c(15,40,25),
                      tahun2016 = c(32, 44, 59),
                      tahun2017 = c(18, 10, 50))
head(laporan)

laporan <- melt(laporan, id.vars = "produk", variable.name = "tahun",
                value.name = "total")

# string manipulation
laporan$tahun <- as.numeric(gsub("tahun", "", laporan$tahun))

ggplot(laporan, aes(x = tahun, y = total)) +
...
ggplot(vids.m, aes(x=category_id, y=value))+
  geom_col(position="stack", aes(fill=vids.m$variable))+
  coord_flip()
```
...
```{r}
ggplot(vids.hate, aes(x=category_id, y=value))+
...
Channel INSIDER
Title dan Jumlah Views Channel INSIDER


```{r}
# vids.insider <- melt(vids.u[vids.u$title], vids.u[vids.u$views])
# 
# vids.hate <- melt(vids.u[vids.u$dislikes >= vids.u$likes & vids.u$ratings_disabled == F, c(4,7,8,9)])
#temp3 <- temp3[order(temp3$dislikeratio, decreasing = T), ]

vids.insider <- vids.u[vids.u$channel_title == "INSIDER", ]
#vids.insider <- vids.insider[order(vids.insider$views, decreasing = T), ]
vids.insider$title <- reorder(vids.insider$title, vids.insider$views)
head(vids.insider)

ggplot(vids.insider, aes(x=title, y= views))+
  geom_col() +
  coord_flip() +
  labs(title = "Jumlah Views Channel INSIDER", x = "Title", y = "Jumlah Views")
```
...
[ggplot2 visualization tips](http://r-statistics.co/Top50-Ggplot2-Visualizations-MasterList-R-Code.html)
