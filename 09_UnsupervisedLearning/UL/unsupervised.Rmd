...
The variables Age and Price in `Sale Price of Vehicle` are not uncorrelated. If you plot a regression line on it, that line exhibit a downward slope, and the correlation between Age and Sales Price would be a value close to -1. 

The variables Temperature and Pressure in `Logistic Machinery` are also not uncorrelated. If you plot a regression line on it, that line exhibit an upward slope, and the correlation between Temperature and Pressure would be a value close to 1. 
...
Let's start by reading in a dataset which is concatenated[^5] from the New York City Department of Finance and made available by City of New York. This dataset is a record of every building or building unit (apartment, etc.) sold in the New York City property market over a 12-month period:  
```{r}
...
ppt <- property %>% 
  ...
  filter(complete.cases(.)) # mengambil data yang tidak mengandung missing value
...
So we've learned that our PCA projects our data onto a new set of dimensions where the variances in our data are captured in a way that allow for meaningful clustering or visualization. We learned also that this new set of dimensions are called "principal components". What do you the correlations between these principal components (say, the correlation between `PC1` and `PC2` are)?

If you're done with the simple experiment above[^8], let's attempt a second question: how many principal components (dimensions) is required to explain ~80% of the variation (round up to closest whole number)? This represent a 60% reduction in dimensionality, which makes PCA a significant and often times critical tool when working with high-dimensional data. 
...
```{r}
data("USArrests") #Data Bawaan dari R
source("biplot.R")
head(USArrests)
```
...
In the second line I use `source("biplot.R")` to read in a function that renders the biplot for you. You can use `fancy_biplot(x)` the same way you would use `biplot(x)`: it accepts a pca object `x` and render a biplot. Now it's your turn: create the PCA object, and use `fancy_biplot()` to render a biplot. 

```{r, fig.width=8, fig.height=8}
USA_prcomp <- prcomp(USArrests, scale. = T, center = T)
fancy_biplot(USA_prcomp)
```
...
2. From the biplot, can you tell which of the four states have a higher proportion of murder / assault / rape? Which of the 4 states have the lowest arrest in all of these three crimes?
Pick from: "Ohio", "Louisiana", "North Dakota" and "Colorado"

Highest proportion of murder: Louisiana
Highest proportion of assault: Lousiana
Highest proportion of rape:  Colorado
Lowest proportion of crime: North Dakota (paling jauh dari arah kriminal)

```{r}
# str(USA_prcomp)
# str(USArrests)

USArrests[c("Ohio", "Louisiana", "North Dakota", "Colorado"),]
```
...
This separation avoids an over-cluttered map and also give us the flexibility to customize each of these two plots individually. 
```{r}
library(FactoMineR)
#loan_pca <- prcomp(loan[,c(3:6,9:11,13:16)], scale=T, center=T)
quantivar <- c(3:6,9:11,14:16)
loan$not_paid <- as.factor(loan$not_paid)
loan[,quantivar] <- scale(loan[,quantivar], center=T, scale=T)
...
`select="contrib 10`: label 10 elements that have the highest contribution on the 2 dimensions of our plot
...
showMatrix(con_manual10, title = 'Reconstructed: 10 dimensions')
...
# Penjelasan
a$clus <- a_k$cluster # Nilai Cluster pada semua variable
a_k$centers # Titik Centroid yang tersedia
a_k$iter # Total Iterasi hingga berhasil
a_k$size # Jumlah data pada masing-masing cluster

a_k$withinss # within ss = sum of square -> menghitung jarak dari setiap variabel ke centroid, dikuadratkan, lalu dijumlahkan. Makin kecil makin baik.
a_k$tot.withinss # jumlah semua withinss
a_k$betweenss / a_k$tot.withinss

a_k

a_k$withinss <- c(2.2,0.9,0.3)
a_k$withinss <- c(1.5,1.2,0.5)
...
a_k$betweenss # jarak centroid ke pusat global

a_k
# Coba buat k sekecil mungkin, tapi between nya sebesar mungkin
```
...
# Subset only for cluster 2
whiskies[whi_km$clust == 2,]
```
...
whiskies[rownames(whiskies) =="Springbank", ]

# Cari cluster nya -> cluster 3
whiskies[rownames(whiskies) == "Laphroig", ]$clust

# Subset only for cluster 3
whiskies[whi_km$clust == 4,]

# So, we can recommend Ardbeg, Caol lia, Clynelish, Lagavulin, Talisker
# So we can use this for Film Recommendation, Book Recommendation, and other recommendation engines.
```
...
```{r}
# creates biplots
whis.pca<-PCA(whi_z, graph = F)
plot(whis.pca, choix=c("ind"), label="none", col.ind=whiskies$clust)
legend("topright", levels(whiskies$clust), pch=19, col=1:4)
```

We observe pretty clear cluster memberships on the two dimensions (PC1 and PC2) - it seems like the two dimensions have captured the variations between these 4 distinct groups of whiskies. It is helpful to recall that prior to our PCA, there is no data reduction yet - k-means was grouping our points with the objective of getting good internal cohesion and external separation **on the original dimensional space!**. The original dimensional space, however, is 13-dimensional and that is difficult to visualize meaningfully. 

Now let's label the point "Laphroig" on our biplot:
```{r}
# get the PC1 and PC2 coordinates for laphroig
indx <- which(rownames(whiskies) == "Laphroig")
laph <- whis.pca$ind$coord[indx,1:2]

# creates individuals factor map
plot(whis.pca, choix=c("ind"), label="none", col.ind=whiskies$clust)

# add points and text
points(laph[1], laph[2], col="dodgerblue4", cex=1.2)
text(laph[1], laph[2]-0.3, col="dodgerblue4", labels = "Laphroig", cex=0.6)
legend("topright", levels(whiskies$clust), pch=19, col=1:4)
```
...
Which of the following three kmeans() model is a better one based on the objective of minimizing the distance between each point and their assigned cluster centres? 
A.  3 clusters; Within-cluster sum of squares of 2.2, 0.9, and 0.3. 
B.  3 clusters; Within-cluster sum of squares of 1.5, 1.2, and 0.5.  Total = 3.2 paling kecil paling bagus
C.  3 clusters; Within-cluster sum of squares of 1.7, 1.5, and 0.5 

Question 2
For a customer who enjoy 'Springbank', which of the following whiskies may be characteristically similar enough to warrant a recommendation? 
A.  Balblair 
B.  Speyburn 
C.  Tomatin (CORRECT)
(Ada yang error dengan dataset gw)

Question 3
Which of the following is NOT TRUE about PCA? 
A.  Because PCA tries to 'summarize' the covariance between variables of our data, it requires that the input variables to be scaled so they have the same range of measurement 
B.  A Principal Component with an eigenvalue of 0.6 is not more helpful than a Principal Component with an eigenvalue of 6.0 in terms of the variance it explains 
C.  We cannot fully reconstruct the original data from a PCA even if we use all the eigenvectors and eigenvalues because PCA always result in some loss of variance / information due to its dimensionality reduction process  (CORRECT)
...
# Enlik's Notes
Iterasi bisa di-set maksimal, menggunakan `kmeans(iter.max = <value>)`
