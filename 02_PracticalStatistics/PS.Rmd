...
```{r}
retail <- read.csv("data_input/workshop.csv", stringsAsFactors = FALSE)
head(retail)
str(retail)
...
# lapply() -> melakukan fungsi pada beberapa kolom/variable sekaligus
retail[, 1:2] <- lapply(retail[, 1:2], as.Date, format = "%Y-%m-%d")
retail[, 1:2]

# lapply() dengan menggunakan nama variabel
retail[,c("Customer.ID", "Product.Name")] <- lapply(retail[, c("Customer.ID", "Product.Name")], as.character)
retail[,c("Customer.ID", "Product.Name")]
...
# ada outlier 105 dan 45, sehingga membuat mean tidak efektif, namun median cukup efektif
mean(salary) #20.03
median(salary) #7.5
sort(salary, decreasing = FALSE)
```
...
```{r}
## Contoh fungsi Modus, karena dalam R tidak ada fungsi default Modus seperti Mean dan Median
...
# Fungsi alternatif yang lebih
most2 <- function(x){
  as.integer(names(sort(table(x), decreasing = TRUE))[1])
}

most(numberoftravels)
most2(numberoftravels)

## Library tambahan untuk menggunakan fungsi bawaan Modus
library(DescTools)
Mode(numberoftravels)
```
...
```{r}
## This 2 functions is same :)
IQR(numberoftravels)
#    ||   #
as.numeric(quantile(numberoftravels, 0.75) - quantile(numberoftravels, 0.25))

## Keep trying
quantile(retail$Profit)
quantile(retail$Profit, probs = c(0, 0.3, 0.9))
```
...
quantile(price.coins)
quantile(price.oil)

sd(price.coins) # 0.6654751
sd(price.oil) # 0.512696 -> Standard Deviation nya lebih kecil dari coins, sehingga Oil lebih 'votality'
```
...
#====Your Solution====
z <- (174-180)/8
pnorm(z)*100 # in percentage
```

**Quiz 3: Supposed we have a population mean of 180cm and standard deviation of 8cm, what is the probability that a randomly picked person being 174 < X < 181?**  
P(X > 174)
P(X < 181)
```{r}
z1 <- (174 - 180)/8
...
pnorm(z1, lower.tail = F) - pnorm(z2)
```
