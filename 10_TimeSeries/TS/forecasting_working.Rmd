...
```{r}
data.frame(year = c(1982, 1986, 1990, 1995,1999,2004,2008), voter.turnout = c(4.3, 4.8,5.7,6.1,6.6,6.9,8.1))
```
...
Notes:
1. parameter harus urut
2. start dan end optional
...
1. What is the class of `presidents`? If it is not a time series object, convert it to a time series object. class(presidents)
...
```{r}
# data presidents sama dengan step sbb:
presidents.vec <- as.numeric(presidents)
length(presidents.vec)
presidents.ts <- ts(presidents.vec, frequency = 4, start = c(1945,1), end = c(1974,4))
presidents.ts
...
plot(presidents)
#simple ggplot
autoplot(presidents) + geom_line(col="tomato3") + theme_bw()
```
...
```{r}
plot(window(presidents, start=c(1960,1), end=c(1970,1)))
```
...
```{r}
births.ts <- ts(data = births$births,
   start = c(1946,1),
   frequency = 12)
autoplot(births.ts)
```
...
```{r}
head(births_ts,12)
head(births_dc$trend,12)

tail(births_ts,12)
tail(births_dc$trend,12)
```
...
plot.ts(births_ts, col="dodgerblue4")
points(births_sma, col="tomato3", lty = 2)
```
...
```{r}
sum(
as.vector(births_ts[3])*0.04166667,
as.vector(births_ts[4:14])*0.08333333,
as.vector(births_ts[15])*0.04166667
)
```


```{r}
d <- decompose(births.ts)


head(births_dc$trend,9)
```
...
detrend <- length(births_ts) - length(births_dc$trend)
...
```{r eval=F}
library(forecast)
plot(decompose(log(souvenir_ts)))
...
plot(co2_ts)
lines(co2_tsm, col="tomato3")
```

```{r}
library(ggplot2)
ggplot(co2, aes(x=year, y=CO2.emissions..metric.tons.per.capita.)) +
  geom_point() +
  geom_smooth()
```
...
length(co2_ts)
future1 <- co2_ts[43]*0.5 + co2_ts[42]*0.2 + co2_ts[41]*0.2 + co2_ts[40]*0.1
future1
```
...
```{r}
co2_ets <- ets(co2_ts, model="ANN")
plot(co2_ts)
lines(co2_ets$fitted, col = "tomato3")
```
...
```{r}
library(lubridate)
seq(from = dmy("12-11-2010"), to=dmy("12-11-2019"))
```
...
```{r eval=F}
co2_ets<- ets(co2_ts, model="ZZZ", alpha=0.2, damped = T)
plot(co2_ts)
lines(co2_ets$fitted, col="tomato3")
```
...
```{r}
plot(rain_ts)
ets.rain <- ets(rain_ts, model="ANN")
lines(ets.rain$fitted, col="tomato3")
ets.rain
```
...
plot(co2_ts)
lines(co2_hw$fitted[,1], col="tomato3")
```
...
```{r}
#sama
ets(rain_ts, model="ANN", alpha = 0.2)
rain_hw <- HoltWinters(rain_ts, alpha = 0.2, beta=F, gamma=F)
```
...
```{r}
ets AAN alpha = 0.4 beta=0.1
ets AAN alpha = 0.6 beta 0.1
...
```{r}
acf(co2_hwf$residuals, na.action = na.pass)
```


```{r}
# use na.action to tell the function that we should just omit the one NA value in our hwf
acf(co2_hwf$residuals, lag.max = 50, na.action=na.pass)
```
...
- Stationary dalam mean / trend konstan
- Staionary dalam variance / tidak ada efek multiplicative dalam seasonality
...
Ho: Tidak Staionary secara variance
...
```{r}
plot(agri_arima$x)
lines(agri_arima$fitted, col="tomato3")
```
...
```{r}
plot(forecast(uscons.arima, h=10))
```

```{r}
uscons.arima1 <- arima(uscons_ts[,1], order = c(3,0,0), seasonal=F)
uscons.arima <- arima(uscons_ts[,1], order = c(0,0,3), seasonal=F)
```
...
```{r}
uscons_arima2 <- Arima(uscons_ts[,1], order=c(3,0,0))
uscons_arima2
```


```{r}
pacf(uscons_arima3$residuals)
```
...
uscons_arima2 <- Arima(uscons_ts[,1], order=c(3,0,0))
uscons_arima2$aic
uscons_arima3 <- Arima(uscons_ts[,1], order=c(0,0,3))
uscons_arima3$aic
```

```{r}
auto.arima(uscons_ts[,1], seasonal = F)
auto.arima(uscons_ts[,1], stepwise = F, approximation = F, seasonal = F)
```
...
```{r}
a <- auto.arima(euretail, stepwise = T, approximation = T)
a
sum(a$residuals^2)
```
...
# Extra Content: 
Using the `wikipediatrend` library, I've downloaded the daily wikipedia views on the English[^11] and Bahasa Indonesia[^12] version of Joko Widodo's article. The full R code is in `wiki.R`. For convenience, I've saved a copy of that dataset into your `data_input` directory:
