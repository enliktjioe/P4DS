...
//Notes:
*Natural Period* yang umum contohnya harian, mingguan, bulanan, tahunan
*Frekuensi* = dalam 1 Natural Period berapa nilainya, contoh: 1 tahun = 365 hari, 1 minggu = 7 hari
...
class(presidents)
```
Time series presidents dengan frekuensi = 4.
...
```{r}
plot(window(presidents, 
              start=c(1963,4),
              end = c(1968,4)))
```
Ini salah satu cara meng-analisa time-series lewat *otak-atik* range untuk melihat tren nya.


4. Was Lydon Johnson a popular president during his presidential term?
NOPE, kita bisa lihat grafiknya menurut


5. As a bonus exercise, give the plot an appropriate main title and axis labels (x and y). 
```{r}
plotly::ggplotly(forecast::autoplot(presidents))
```

Data Year, sample per bulan, frequency daily
```{r}
365.25/12
```
...
```{r}
births_ts <- ts(births$births,
    start = c(1946,1),
    frequency = 12)
forecast::autoplot(births_ts)
...
#NOTES
- Februari adalah nilai seasonal terkecil
- July adalah nilai seasonal terbesar
...
```{r eval=F}
plot(decompose(souvenir_ts, type = "multiplicative"))
...
co2_ets$residual
```
`ANN` means ADDITIVE for Error ; NONE for Trend ; NONE for Seasonality
`fitted` means hasil smoothing 
`residual` 
...
#Cheatsheet
1. Error -> A (Additive) / M (Multiplicative) / Z (Automatic Model, yang terbaik)
2. Trend -> A / Ad (Teredam) / Z
3. Seasonality -> A (Additive) / M (Multiplicative) / Z
...
```{r eval=F}
co2_ets<- ets(co2_ts, model="ZZZ", alpha=0.2)
```
