...
# length(nneu$net.result) # melihat nilai repetisi yang adalah 5
```
...
unlist(lapply(nneu$net.result, sse))/2
```
...
data.frame("error_compute" = nneu_compt$net.result, "error_nn_3"=nneu$net.result[[3]])
```
...
- [NO] Change `err.fct` from `ce` to `sse` 
...
With our `fomula` and `data` ready, we can now implement our neural network. I set the cost function (`err.fct`) to be the cross-entropy function, and because it's a classification algorithm we set `linear.output` to `FALSE`. The `threshold` is set to 0.1; Our multi-layer perceptron requires the model's partial derivative of our cost function to be at least 0.1; otherwise, it should stop "descending":
```{r eval=F}
system.time({
  set.seed(100)
  bankmar_nn <- neuralnet(formula = fml, data = bank_m, 
                          hidden = c(5,3), rep = 1, threshold = 0.1, err.fct = "ce",
                          linear.output = F)
})
...
```{r eval=F}
saveRDS(bankmar_nn, file = "bank_marketing_nn.rds")
```
...
```{r fig.height=3}
bankmar_nn2 <- readRDS("bank_marketing_nn.rds")
...
# [Understanding-Only] Apache MXNet 
...
m1 <- mx.model.FeedForward.create(m1.softmax,  #the network configuration made above
                                     ...
                                     array.batch.size = 80, # RAM gak cukup, batch.size nya diperkecil
                                     momentum = 0.95,
...
In the near future, I'll release a mini-series focused on all the ways to add performance to your machine learning models and I'll make sure you receive a copy of it. Within the constraints of what is possible in 90 hours of hands-on coding and training, I do hope that you have gained precious knowledge throughout this Specialization. At the very least, I hope it has been both educational and fun (in a weird way) while learning how to use mathematical tools in solving prediction problems. 
