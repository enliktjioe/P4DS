...
Another example where logistic models are frequently applied is at modeling customer churn and attrition[^3]:
...
set.seed(10)
wholesale.n1 <- wholesale.n[sample(nrow(wholesale.n)), ]
# 80% train - 20% test
wholesale_train <- wholesale.n1[1:352, ]
wholesale_test <- wholesale.n1[353:440, ]
table(wholesale_train$Industry)
```

```{r}
horeca_pred <- class::knn(train = wholesale_train[,1:6],
                   test=wholesale_test[,1:6],
                   cl=wholesale_train$Industry,
                   k=15)

table("actual" = wholesale_test$Industry, "predicted" = horeca_pred)
```
...
```{r}
data1 <- read.csv("data_input/sample.csv")
set.seed(218)
idx <- sample(nrow(data1), nrow(data1) * 0.8)

data1.train <- data1[idx,]
data1.test <- data1[-idx,]
