...
```{r}
write_RDS(modelForest, "modelRanger.RDS")
```


```{r}
# modelForest <-readRDS("modelforest.RDS")
# modelForest
```
...
```{r}
table(predict(modelForest, sms.test[,-57]), sms.test[,57])
```

```{r}
sum(predict(modelForest, sms.test[,-57]), sms.test[,57])
```

```{r}
plot(modelForest$finalModel)
legend("topright", colnames(modelForest$finalModel$err.rate),col=1:6,cex=0.8,fill=1:6)
```
